...
|-----|---------|---------------|
| `file_types` | one of the `_classify_file` labels (`pdf`, `image`, `video`, `audio`, `text`, `json`, `table`, `code`, `office`, `html`, `xml`, `yaml`, `notebook`, `ebook`, `log`, `unknown`) | `metadata.file_type` (stamped from the document source at ingest; backfilled onto older datasets by the admin endpoint below) |
| `severities` | any observed severity (`ERROR`, `WARN`, ...) | `metadata.severities` |
...
- **Weighted RRF (per-dataset defaults)**: each lane's rank positions can be tilted with per-dataset weights — `meta.json` gains `rrf: {dense_weight, sparse_weight, k}` (settable at create time via `POST /api/datasets {"rrf": …}`, later via `PATCH /api/datasets/{name}`, and from the home page's dataset form). Rank-space tilt, NOT a score multiplier: qdrant scores each lane `1/((pos+1)/w + k − 1)` — trust the order, not the magnitude. Resolution order per search: explicit caller override (`dense_weight`/`sparse_weight`/`k` on the REST/MCP search surfaces) **>** dataset default **>** global default (1.0/1.0, unpinned k). A dataset default equal to the global default is stored as *absent* and behaves byte-identically to before (the request keeps the unpinned `FusionQuery` form — pinning k would silently change rank arithmetic). The response's `rrf` block (`{dense, sparse, k, applied}`) reports the EFFECTIVE parameters; `applied=false` on dense-degraded searches. Multimodal (vector-only) queries ignore weights (no fusion to weight). **Federated search ignores per-dataset defaults by ruling** — one merged pool cannot honour differing per-dataset weights; only an explicit caller override may apply there (the federated surfaces deliberately take no weight parameters in this slice). Deployment-level create-time default: `rag.rrfDefault` chart values (`RAG_RRF_DEFAULT` env, `dense,sparse[,k]`) stamp NEW datasets when enabled — disabled by default, existing datasets untouched.
...
### Contextual retrieval (ingest-time document context)

Dense and lexical retrieval both degrade on isolated chunks — a mid-document page carries no hint of *which* document it belongs to. Contextual retrieval (DECISIONS.md 2026-09) fixes that at INGEST time: for every chunk that carries real extracted text, one small LLM call (`rag.vlm` — the same text-capable ChatModel the caption path uses; there is deliberately **no new model role**) writes 1–2 sentences of document-level context, and the sentence is prepended to the chunk text as a `[Document context]: …` line **before embedding**. The line travels with the chunk everywhere the text does: the dense vector, the BM25 sparse lane (`_bm25_indexable_text` keeps it — only bare media placeholders are stripped), and the stored payload where the retrieval-time LLM can quote it (visibly labeled, so answers can attribute).

- **Gating (three levels, disabled by default everywhere)**: the chart knob `rag.contextual` (`RAG_CONTEXTUAL_DEFAULT` env) only pre-checks the create form / stamps NEW datasets at create time — a disabled deployment renders no config key and behaves byte-identically; the per-dataset `meta["contextual"]` flag (`create_dataset(contextual=…)` / `PATCH /api/datasets/{name} {"contextual": …}` — rides the caption-flag RAG-cache invalidation) is what actually gates the pipeline; and the module is a no-op without a VLM. An explicit `contextual` in the create body always wins over the env.
- **The honest enable story**: the flag affects NEW ingests only. Re-uploading existing files after enabling hits `.ingested_hashes.json` content dedup — the context changes the embedding, not the file hash — so stored content is NOT re-contextualized by re-upload. **Run Recreate** (`POST /api/admin/datasets/{name}/recreate`) to re-contextualize existing files; mixed context modes inside one dataset are allowed by design.
- **Cost**: one small LLM call per real-text chunk (on 8k-page PDFs at 2048-token chunks that is thousands of calls). `POST /api/admin/datasets/{name}/contextual-preview` returns a LABELED ESTIMATE before the flag flips: live Qdrant point count (×2 for hybrid-capable collections — real-text chunks get text-only twins carrying the same line) × (shared preamble + chunk input + ~80 output tokens) + the configured VLM name; the dashboard shows it as the confirm line next to the checkbox.
- **Prefix caching**: the prompt is `system (constant) + user (doc preamble + chunk)`, and the preamble (filename + first-N-chars excerpt, `RAG_CONTEXTUAL_DIGEST_CHARS`, default 512) is byte-identical for every chunk of one document. Chunks are contextualized per document in producer order (wave-4 sequencer order — never gathered across files), and each document's first call completes before its siblings fan out, so vLLM/SGLang prefix caching absorbs the shared prefix. In-flight calls are bounded by `RAG_CONTEXTUAL_CONCURRENCY` (default 8, `<= 0` unbounded — the `MODEL_EMBED_MAX_CONCURRENCY` semaphore pattern).
- **Skips** (verified interplay gotchas): pure-media docs (no `_has_real_text` — a context line would newly count as real text and flip the text-only-twin gating; media docs already have the caption-twin path), `memory_kind`-tagged docs (memories stay verbatim), and pre-made twins. Raw-document chunks longer than ~6k chars get a prompt-capped excerpt but store the full text.
- **Fail-open**: any LLM error stores the plain chunk, stamps `contextualized: false`, and records a non-fatal ingest warning (the Preprocessor caption-skip discipline) — a VLM outage degrades ingest to un-contextualized, never fails it.
- **Introspectability**: every stored point carries `metadata.contextualized: bool`; `meta.json` records `contextual` next to the embedder fingerprint (`_write_embedder_fingerprint`), and a later mode mismatch WARNS (never errors — mixed-embedding datasets are explicitly permitted; Recreate is the way to make a dataset uniform). **`DATASET_SCHEMA_VERSION` is intentionally NOT bumped** — no collection-shape change; a bump would fire schema-upgrade recreates for every dataset.
...
| POST | `/api/datasets` | Create (name, description, caption_with_asr, caption_with_vlm, keep_originals, ocr, password) |
...
| POST | `/api/datasets/{name}/batch-urls` | S3/HTTP URL ingestion with SSE streaming; `sync: true` reconciles an S3 prefix (prunes sources deleted upstream; unchanged objects skipped via the `.watched_state.json` ETag/Size state), `sync_dry_run: true` reports the diff (`would_ingest`/`would_prune`/`would_download`/`unchanged`) without touching anything |
...
Exposes **19 MCP tools** (`mcp_server.py`):
...
`GET /metrics` exposes process-local Prometheus counters/histograms (`utils/metrics.py`, import-optional — minimal installs degrade to no-ops):

- **HTTP**: `rag_http_requests_total` + `rag_http_request_seconds` by route template/method/status (outermost ASGI middleware — timings include auth; unmatched paths are labelled `unmatched`).
- **Ingest**: `rag_ingest_files_total` by outcome (stored/deduplicated/error), `rag_ingest_chunks_total` per dataset, `rag_ingest_jobs_total` by source/terminal state.
...
**Fail-closed default (D20, 2026-09-24):** a deployment with NO `RAG_API_KEY` and no key registry no longer runs open — unauthenticated requests bind an anonymous registry-client identity whose ONLY grant is the deployment's memory dataset (`MEMORY_DATASET` env, when set): dataset paths 403 (401 at the memory dataset's own password gate), `/api/admin/*` 403, dataset creation 403, while the memory tools keep working. Public surfaces (`/`, `/manage`, `/access`, `/healthz`, `/readyz`, `/metrics`, self-authorizing media/staged serving) are unchanged. Any configured key (REST, MCP keyset, registry, overlay) restores the normal resolution.

**Multi-user registry keys (D15/D16/D17):** `RAG_API_KEY_CLIENTS` / `RAG_DATASET_ACLS` mint per-user keys with per-dataset ACLs (fail-closed; admin surfaces denied to registry keys; **D19**: through the PCAI LLM gateway the explicit `X-API-Key` delegated identity wins over the co-forwarded admin `Authorization: Bearer`, so per-user ACLs work behind the gateway). With the D16 access store on (`RAG_ACCESS_STORE=1`, chart default `security.accessStore: true`), keys self-select datasets (ACL ∪ selections) and bind a ★ memory dataset; an admin key can mint/grant/rotate/revoke per-user keys from the `/access` **User keys** panel (D17, file-backed overlay at `{DATA_PATH}/access/clients.json` merged with the env registry). Full semantics: README § Security hardening, API.md § 1.
...
- `INGEST_ALLOW_HOSTS` is an authoritative allowlist: listed hosts bypass the private-block (how in-cluster MinIO/internal ingestions are permitted); when set, hosts not listed are rejected.
...
- **S3 directory prefixes** (`s3://bucket/prefix/`): lists all supported-type objects under the prefix and ingests them as a batch.
  ...
  - **Watched-source skip state** (`files/.watched_state.json`, opt-in feature): on sync paths the batch consults a per-dataset sidecar recording, per object key, the ETag+Size of the last successfully ingested version. A listed object whose ETag+Size still matches is dropped from the download set *before* any bytes move (no re-download, no re-embed attempt); the state is written after the batch for every object that demonstrably ingested (stored chunks or a content-hash dedup skip), so a failed/zero-output object is re-attempted next tick instead of silently skipped. Content changed under the same key flips the ETag and re-ingests. Sync's prune forgets the fingerprints of upstream-deleted sources (a pruned-and-reappeared object re-ingests). The dry-run reports the split as `would_download` vs `unchanged`. The state is an optimisation, never a correctness gate: a corrupt/missing file degrades to re-downloading everything, and once state exists, deleting a source's documents out-of-band sticks (the reconciler no longer resurrects them). Dataset delete/Recreate clears it. Consumed automatically by the watched-sources CronJob (chart values `watchedSources.*`).
...
**Per-dataset metadata** (`meta.json`): `name`, `description`, `caption_with_asr`, `caption_with_vlm`, `created`, `document_count`, `password_hash`, `file_type_counts` (dict of per-type doc counts), `ocr` (scanned-page fallback), `rrf` (per-dataset weighted-RRF defaults, when set), `contextual` (ingest-time contextual retrieval), and the embedder fingerprint (`embedder_model`, `embedder_dim`, `contextual` mode, `embedder_updated_at`).
...
/api/datasets/{name}/upload-status/{job_id}`). The dataset's contextual-retrieval mode rides the same fingerprint (`contextual`): a later mode flip is **warned about, never an error** — mixed context modes inside one dataset are permitted by design, and per-point `metadata.contextualized` records which points are which. **Alias groups** (`models.embedder.extra.alias` — a model id, list, or comma-separated string) declare ids that must be treated as THE SAME embedder (e.g. a quantized FP8 redeploy of the same base model): datasets indexed under an aliased id keep working after the swap without a recreate; aliases excuse the model name only — a dimension mismatch still fails loudly.
