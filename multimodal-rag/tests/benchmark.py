...
async def check_health(base_url: str, insecure: bool = False) -> bool:
    ...
    try:
        async with httpx2.AsyncClient(timeout=10, verify=not insecure) as c:
...
async def discover_datasets(
    base_url: str, password: str | None = None, insecure: bool = False, api_key: str | None = None
) -> list[str]:
...
async def run_benchmark(args: argparse.Namespace) -> None:
    ...
    if is_mcp:
        ...
        async with httpx2.AsyncClient(limits=limits, http2=args.http2, verify=not args.insecure) as client:
            tasks = [
                run_user(
                    user_id=i,
                    client=client,
